"""
PRISM Live Room v2 — Real-time zonal radar with v2 features
Uses prism_model_v2.pkl (GradientBoosting, 87-dim features)
"""
import serial
import numpy as np
import pandas as pd
...
# Load v2 model
try:
    clf = joblib.load("prism_model_v2.pkl")
    print("🧠 PRISM AI v2 Model Loaded Successfully!")
except FileNotFoundError:
    print("❌ ERROR: 'prism_model_v2.pkl' not found. Run prism_ai_v2.py first!")
    exit()

# Must match NULL_SUBCARRIERS in prism_ai_v2.py
NULL_SUBCARRIERS = list(range(27, 38))
ACTIVE_COLS = [i for i in range(64) if i not in NULL_SUBCARRIERS]

# ==========================================
# DSP — identical to training
# ==========================================
...
# ==========================================
# FEATURE EXTRACTION — must match prism_ai_v2.py exactly
# ==========================================
...
def extract_features(w):
    ...
    f.extend(_agg(np.sum(fft_p*fa[:,None],0)/(np.sum(fft_p,0)+1e-10)))
    ...
    # Top-10 per-subcarrier variance + energy
    var_per_sc = np.var(w, axis=0)
    top10_idx = np.argsort(var_per_sc)[::-1][:10]
    f.extend(var_per_sc[top10_idx].tolist())
    f.extend(np.mean(w**2, axis=0)[top10_idx].tolist())

    vec = np.nan_to_num(np.array(f), nan=0.0, posinf=0.0, neginf=0.0)
...
# ==========================================
# GUI SETUP (3 zones, not 4)
# ==========================================

plt.ion()
fig, ax = plt.subplots(figsize=(10, 6))
fig.canvas.manager.set_window_title('Project PRISM v2: Live Radar')
plt.style.use('dark_background')
ax.set_xlim(0, 10)
ax.set_ylim(0, 6)
ax.set_title("Project PRISM v2: Live Wi-Fi Zonal Tracking", fontsize=18, fontweight='bold', color='white')
ax.axis('off')

zones = {
    0: [0,   0,   10, 6, "EMPTY ROOM",    "gray"],
    1: [0.5, 0.5,  4, 5, "Zone A",        "cyan"],
    2: [5.5, 0.5,  4, 5, "Zone B",        "orange"],
}
...
try:
    while True:
        ...
        # Run inference on EVERY packet (no slide gate)
        ...
        # Update display immediately when votes agree
        ...
        # Always update GUI on every packet
